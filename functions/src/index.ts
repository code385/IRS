...
const RESEND_FROM        = process.env.RESEND_FROM        ?? "IRS Timesheet <onboarding@resend.dev>";
const RESEND_TEMPLATE_ID = process.env.RESEND_TEMPLATE_ID ?? "";
const LARK_SMTP_USER     = process.env.LARK_SMTP_USER     ?? "";
const LARK_SMTP_PASS     = process.env.LARK_SMTP_PASS     ?? "";
const GMAIL_USER         = process.env.GMAIL_USER         ?? "";
const GMAIL_APP_PASSWORD = process.env.GMAIL_APP_PASSWORD ?? "";
const VERIFY_URL_BASE    = process.env.VERIFY_URL_BASE    ?? "https://us-central1-hashtimesheet.cloudfunctions.net";
...
function requireEnv(name: string, value: string): string {
  if (!value) {
    throw new HttpsError(
      "failed-precondition",
      `Missing environment variable: ${name}. Add it to functions/.env and redeploy.`
    );
  }
  return value;
}
...
// ─────────────────────────────────────────────────────────────────────────────
export const sendCredentialsEmail = onCall(
  async (req: CallableRequest) => {
    if (!req.auth) {
      throw new HttpsError("unauthenticated", "You must be signed in.");
    }

    const { to, name, password, role } = req.data as {
      to: string; name: string; password: string; role: string;
    };

    if (!to || !name || !password || !role) {
      throw new HttpsError("invalid-argument", "Missing: to, name, password, or role.");
    }
...
// ─────────────────────────────────────────────────────────────────────────────
export const createUserAndSendCredentials = onCall(
  async (req: CallableRequest) => {
    if (!req.auth) {
      throw new HttpsError("unauthenticated", "You must be signed in.");
    }

    const { email, password, name, role } = req.data as {
      email: string; password: string; name: string;
      role: "Super Admin" | "Admin" | "Manager" | "Employee";
    };

    if (!email || !password || !name || !role) {
      throw new HttpsError("invalid-argument", "Missing: email, password, name, or role.");
    }
    ...
    if (callerRole !== "Admin" && callerRole !== "Super Admin") {
      throw new HttpsError("permission-denied", "Only Admin or Super Admin can create users.");
    }
    if ((role === "Admin" || role === "Super Admin") && callerRole !== "Super Admin") {
      throw new HttpsError("permission-denied", "Only Super Admin can create Admin or Super Admin accounts.");
    }
...
export const sendVerificationEmail = onCall(
  async (req: CallableRequest) => {
    if (!req.auth) {
      throw new HttpsError("unauthenticated", "Must be signed in to request email verification.");
    }
    ...
    if (!email) {
      throw new HttpsError("invalid-argument", "The account has no email address.");
    }
...
export const verifyEmailToken = onRequest(async (req, res) => {
  ...
  await auth.updateUser(data.uid as string, { emailVerified: true });
...
export const resendVerificationEmail = onCall(
  async (req: CallableRequest) => {
    ...
    const email = userRecord.email;
    if (!email) {
      throw new HttpsError("invalid-argument", "No email address on this account.");
    }
...
// ─────────────────────────────────────────────────────────────────────────────
export const markUserEmailVerified = onCall(
  async (req: CallableRequest) => {
    if (!req.auth) {
      throw new HttpsError("unauthenticated", "You must be signed in.");
    }
    ...
    if (!isSelf) {
      ...
      const callerRole = callerDoc.data()?.role as string | undefined;
      if (callerRole !== "Admin" && callerRole !== "Super Admin") {
        throw new HttpsError("permission-denied", "Not authorized to verify this user.");
      }
    }
...
export const deleteAuthUser = onCall(
  async (req: CallableRequest) => {
    ...
    if (callerRole !== "Admin" && callerRole !== "Super Admin") {
      throw new HttpsError("permission-denied", "Only Admin or Super Admin can delete users.");
    }
    ...
    if (callerRole === "Admin" && (targetRole === "Admin" || targetRole === "Super Admin")) {
      throw new HttpsError("permission-denied", "Admin cannot delete Admin or Super Admin accounts.");
    }
 ...
 */
async function sendVerificationEmailMultiChannel(
  uid: string,
  email: string,
  name: string
): Promise<void> {
  ...
  throw new HttpsError("unavailable", "All email channels failed. Client should use Firebase native sendEmailVerification.");
}

const TOKEN_EXPIRY_MS = 60 * 60 * 1000; // 1 hour

/** Generates a cryptographically secure token and persists it to Firestore. */
async function storeVerificationToken(uid: string, email: string): Promise<string> {
...
function buildCredentialsEmailHtml(name: string, email: string, password: string, role: string): string {
  return `<!DOCTYPE html>
<html lang="en">
...
function verifyPage(type: "success" | "error", title: string, message: string): string {
